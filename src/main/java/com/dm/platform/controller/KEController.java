    ...
    public void file_manager(HttpServletRequest request, HttpServletResponse response)
        ...
        String rootPath =
            webappDir + htmlFolder + resource + uploadPath+ "/" + UserAccountUtil
                .getInstance().getCurrentUserId() + "/";
        // 文件保存目录URL
        String rootUrl =htmlFolder+resource+
        		uploadPath + "/" + UserAccountUtil.getInstance()
                .getCurrentUserId() + "/"; //"D:\\workspace\\space1\\.metadata\\.me_tcat7\\webapps\\html\\resourceupload\\tXjFPAjeM5vV9yal\\";//rootPath;
